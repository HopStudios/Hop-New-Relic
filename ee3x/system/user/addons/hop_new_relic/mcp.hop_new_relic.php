...
class Hop_new_relic_mcp
{
	...
	*/
	...
	{	
		...
		$sidebar->addHeader(lang('server_data'), ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME));
		$sidebar->addHeader(lang('settings'), ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME.'/settings'));
		$sidebar->addHeader(lang('custom_datasets'), ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME.'/custom_datasets'))
			->withButton(lang('new'), ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME.'/new_custom_dataset'));
	...
	function index()
	{
		...
		$vars = array();
		$vars['chart_form_action'] = ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME.'/get_nr_metrics')->compile();
		$vars['datasets'] = array();
		$vars['errors'] = array();
		...
		return array(
			'heading'		=> lang('server_data'),
			'body'			=> ee('View')->make(HOP_NEW_RELIC_NAME.':index')->render($vars),
			'breadcrumb'	=> array(
	...
	function settings()
	{
		$this->build_nav();
		$header = array(
			'title' 	=> lang('hop_new_relic_module_name'),
		);
		...
		if (ee()->input->post('action') == "save_settings")
		{
			...
			if ($form_is_valid)
			{
				...
				ee('CP/Alert')->makeInline('shared-form')
					->asSuccess()
					->withTitle(lang('success_alert_title'))
					->addToBody(lang('settings_saved_success'))
					->defer();

				ee()->functions->redirect(ee('CP/URL')->make('addons/settings/'.HOP_NEW_RELIC_NAME.'/settings'));
			}
			else
			{
		...
		$vars['refresh_app_list_link'] = ee('CP/URL')->make('addons/settings/'.HOP_NEW_RELIC_NAME.'/refresh_apps_list');
		$vars['refresh_server_list_link'] = ee('CP/URL')->make('addons/settings/'.HOP_NEW_RELIC_NAME.'/refresh_servers_list');
		$vars['action_url'] = ee('CP/URL')->make('addons/settings/'.HOP_NEW_RELIC_NAME.'/settings');
		...
		return array(
			...
			'body'				=> ee('View')->make(HOP_NEW_RELIC_NAME.':settings')->render($vars),
			'breadcrumb'	=> array(
			  ee('CP/URL', 'addons/settings/'.HOP_NEW_RELIC_NAME)->compile() => lang('hop_new_relic_module_name')
			),
		);
	...
	function refresh_apps_list()
	{
		...
		if ($nr_api_key != null && $nr_api_key != "")
		{
			...
			if (!empty($apps))
			{
				...
				ee('CP/Alert')->makeInline('shared-form')
					->asSuccess()
					->withTitle(lang('success_alert_title'))
					->addToBody(lang('apps_list_refresh_success'))
					->defer();
			}
			else
			{
				...
				ee('CP/Alert')->makeInline('shared-form')
					->asIssue()
					->withTitle(lang('error_refreshing_apps_list'))
					->addToBody(lang('apps_list_refresh_failed'))
					->defer();
			}
		}
		else
		{
			...
			ee('CP/Alert')->makeInline('shared-form')
				->asIssue()
				->withTitle(lang('error_refreshing_apps_list'))
				->addToBody(lang('apps_list_refresh_failed_no_key'))
				->defer();
		}
	...
	function refresh_servers_list()
	{
		...
		if ($selected_app != null && $nr_api_key != null && $nr_api_key != "")
		{
			if (isset($selected_app->links) && is_array($selected_app->links->servers))
			{
				...
				if (!empty($servers_result))
				{
					...
					ee('CP/Alert')->makeInline('shared-form')
						->asSuccess()
						->withTitle(lang('success_alert_title'))
						->addToBody(lang('app_servers_list_refresh_success'))
						->defer();
				}
				else
				{
					ee('CP/Alert')->makeInline('shared-form')
						->asIssue()
						->withTitle(lang('error_refreshing_servers_list'))
						->addToBody(lang('apps_servers_received_empty_list'))
						->defer();
				}
			}
			else
			{
				ee('CP/Alert')->makeInline('shared-form')
					->asIssue()
					->withTitle(lang('error_refreshing_servers_list'))
					->addToBody(lang('apps_no_servers_list'))
					->defer();
			}
		...
		else if ($serialized_selected_app == null || $serialized_selected_app == "")
		{
			ee('CP/Alert')->makeInline('shared-form')
				->asIssue()
				->withTitle(lang('error_refreshing_servers_list'))
				->addToBody(lang('servers_list_refresh_no_app'))
				->defer();
		}
		else if ($nr_api_key == null || $nr_api_key == "")
		{
			ee('CP/Alert')->makeInline('shared-form')
				->asIssue()
				->withTitle(lang('error_refreshing_servers_list'))
				->addToBody(lang('servers_list_refresh_failed_no_key'))
				->defer();
		}
		ee()->functions->redirect(ee('CP/URL')->make('addons/settings/'.HOP_NEW_RELIC_NAME.'/settings'));
	}
